...
RUN cat /usr/src/app/sql/stitch_to_umls.sql >> /docker-entrypoint-initdb.d/001structure.sql
...
RUN cat /usr/src/app/sql/drugbank_id_mapping.sql >> /docker-entrypoint-initdb.d/001structure.sql
